...
function init() {
	...
	selectiveBloomPass = new EffectPass(camera, selectiveBloomEffect);
	...
	const bakedTexture = textureLoader.load(
		require("../assets/VRWorld/baked5.webp"),
	);
	bakedTexture.flipY = false;
	bakedTexture.encoding = THREE.sRGBEncoding;

	const bakedDirTexture = textureLoader.load(
		require("../assets/VRWorld/DirectionTexture.webp"),
	);
	bakedDirTexture.flipY = false;
	bakedDirTexture.encoding = THREE.sRGBEncoding;

	const bakedMaterial = new THREE.MeshBasicMaterial({ map: bakedTexture });
	const bakedDirMaterial = new THREE.MeshBasicMaterial({
		map: bakedDirTexture,
	});
	const doorLightMaterial = new THREE.MeshBasicMaterial({
		color: 0xffffff,
	...
	//World Model
	loader.load(
		require("../assets/VRWorld/portfolio_v3_bigItems.glb").default,
		function (gltf) {
	...
	loader.load(
		require("../assets/VRWorld/portfolio_v4_smalllItems.glb").default,
		function (gltf) {
			gltf.scene.traverse((child) => {
				if (child.type === "Mesh") {
					const box = new THREE.Box3().setFromObject(child);
	...
	//Character Model
	loader.load(
		require("../assets/Character/character3.glb").default,
		function (gltf) {
...
function render() {
	...
	if (characterControllerInstance) {
		...
		raycaster.set(
			new THREE.Vector3(
				characterControllerInstance.character.position.x,
				characterControllerInstance.character.position.y + 4,
				characterControllerInstance.character.position.z,
			),
...
function onClickEventHandler(event) {
	...
	if (INTERSECTED) {
		...
		mouse.x =
			((clickxy.x - renderer.domElement.offsetLeft) /
				renderer.domElement.clientWidth) *
				2 -
			1;
		mouse.y =
			-(
				(clickxy.y - renderer.domElement.offsetTop) /
	...
	if (intersects.length > 0) {
		...
		if (INTERSECTED != intersects[0].object) {
			...
			gsap.to(INTERSECTED.scale, {
				duration: 0.5,
				ease: "ease.in(1)",
				z: INTERSECTED.userData.scaleZ + 1,
			});
		}
	} else {
		if (INTERSECTED) {
...
function onClose() {
	selectItemSound.play();
	gsap.to(".box, .library, .lab, .next, .prev, .instructions", {
		zIndex: -1,
		opacity: 0,
		duration: 0.5,
	});
		...
		if (INTERSECTED.userData.group == "projects") {
			...
			if (INTERSECTED.name == "twitter") {
				...
				window.location.href =
					"mailto:thebenezer.mail@gmail.com?subject=Contacting from Portfolio";
			}
		}
	}
	// console.log(INTERSECTED)
...
function setUpRayInterractions() {
	...
	const clickTexture = textureLoader.load(
		require("../assets/images/click.webp"),
	);
	...
	proj1.position.set(5.41, 1.9, -26.33);
	proj1.name = "proj1";
	...
	proj4.position.set(-15.26, 1.9, -41.01);
	proj4.name = "proj4";
	...
	proj5.position.set(-9.82, 1.9, -26.55);
	...
	lightHouse.position.set(22, 1.9, 77);
	...
	lab.position.set(-26.27, 1.9, 53.14);
	lab.name = "lab";
	...
	social1.position.set(37.9, 1.9, 32);
	...
	social4.position.set(54.7, 1.9, 32);
	social4.name = "mail";
		...
		() => {
			gsap.delayedCall(1, () => {
				...
				document
					...
					.addEventListener("click", openPortfolio, { once: true });
...
function openPortfolio() {
	...
	gsap.delayedCall(1, () => {
		...
		var tl = new gsap.timeline({
			...
			repeatDelay: 0.5,
			onComplete: () => {
				gsap.to(".instructions_wiggle, .instructions_pointer", {
					opacity: 0,
					duration: 0.5,
				});
				gsap.to(".instructions_wiggle, .instructions_pointer", {
...
function setupGlobalAudio() {
	...
	audioLoader.load(
		require("../assets/sounds/mainselectsound.mp3"),
		function (buffer) {
	...
	audioLoader.load(
		require("../assets/sounds/itemselectsound.wav"),
		function (buffer) {
