...
portalAnimation.to(".portalOverlay", {
	duration: 0.5,
	width: "100%",
	height: "100%",
	borderRadius: 0,
...
export default class CharacterController {
	...
	update(delta) {
		...
		if (this.body.position.y < 2.4) {
			...
			if (
				(directionPressed && this._input.shiftToggle) ||
				this._input._inputTouch.touchRun
			) {
				play = "run";
		...
		if (
			(this._input.keysPressed[" "] || this._input._inputTouch.touchJump) &&
			this.canJump
		) {
